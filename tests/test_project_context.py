...
MODULE = importlib.import_module('agents.openimpact_dispatcher')
...
class ProjectContextTest(unittest.TestCase):
    ...
    def test_primary_names_match(self) -> None:
        spec = MODULE.build_project_spec()
        self.assertEqual(spec.primary_python_module, 'openimpact_dispatcher')
        self.assertEqual(spec.primary_contract_name, 'OpenServJobBoard')
