...
contract OpenServJobBoardTest is Test {
    OpenServJobBoard private controller;
    ...
    bytes32 private actionId = keccak256("synthesis_openserv");
    bytes32 private subjectId = keccak256("subject");

    function setUp() public {
        controller = new OpenServJobBoard(admin, operator, reporter, 100 ether);
        vm.startPrank(admin);
    ...
    function testExecuteBoundedActionRevertsWithoutReportedBalance() public {
        OpenServJobBoard freshController = new OpenServJobBoard(admin, operator, reporter, 100 ether);
        vm.startPrank(admin);
